...
const requester = { email: "sarah.johnson@kmutt.ac.th", password: "DevPass123!" };
...
async function createRequesterFixture() {
  ...
  try {
    const login = await api.post("/api/auth/login", { data: requester });
    ...
    return { id: (await created.json()).id as number, summary };
  } finally {
    await api.dispose();
  }
}
