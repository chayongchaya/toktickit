...
const priorityTokens: Record<string, React.CSSProperties> = {
  HIGH: { backgroundColor: "#FDE8E8", color: "#9B1C1C", border: "1px solid #F8B4B4" },
  MEDIUM: { backgroundColor: "#FEF08A", color: "#854D0E", border: "1px solid #FDE047" },
  LOW: { backgroundColor: "#DEF7EC", color: "#03543F", border: "1px solid #BCF0DA" },
};
const statusTokens: Record<string, React.CSSProperties> = {
  NEW: { backgroundColor: "#F3F4F6", color: "#374151", border: "1px solid #E5E7EB" }, OPEN: { backgroundColor: "#EBF5FF", color: "#1E429F", border: "1px solid #C3DDFD" },
  ...
  RESOLVED: { backgroundColor: "#DEF7EC", color: "#03543F", border: "1px solid #BCF0DA" }, CLOSED: { backgroundColor: "#E5E7EB", color: "#1F2937", border: "1px solid #D1D5DB" }, REOPENED: { backgroundColor: "#FFEDD5", color: "#9A3412", border: "1px solid #FED7AA" }, CANCELLED: { backgroundColor: "#FDE8E8", color: "#9B1C1C", border: "1px solid #F8B4B4" },
};
const roleTokens: Record<string, { label: string; backgroundColor: string; color: string; border: string }> = {
  REQUESTER: { label: "Requester", backgroundColor: "#F3F4F6", color: "#374151", border: "#E5E7EB" },
  IT_STAFF: { label: "IT Staff", backgroundColor: "#EBF5FF", color: "#1E429F", border: "#C3DDFD" },
  ADMINISTRATOR: { label: "Administrator", backgroundColor: "#006B3C", color: "#FFFFFF", border: "none" },
};
...
export const StaffTicketDetailPage: React.FC = () => {
  ...
  const save = async (kind: "owner" | "priority" | "status", label: string, action: () => Promise<Ticket>) => {
    setSaving(kind); setFieldError(null); setSuccess(null); setFieldSuccess(null);
    try { setTicket(await action()); setSuccess(`${label} saved.`); setFieldSuccess(kind); }
